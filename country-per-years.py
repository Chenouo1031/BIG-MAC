...
# 選擇需要的欄位，並移除缺失值
df_plot = df_2025[['years',x]].dropna()  #篩選所需數值

# 設定圖形大小
plt.figure(figsize=(10, 6))   #設定圖表大小

# 生成漸層顏色
norm = plt.Normalize(df_plot[x].min(), df_plot[x].max())  #標準化數值範圍，設定最大值和最小值
sm = plt.cm.ScalarMappable(cmap="coolwarm", norm=norm)  #使用coolwarm顏色來展示圖表
colors = [sm.to_rgba(val) for val in df_plot[x]]   #將數值轉換為對應的顏色

# 使用 Seaborn 繪製長條圖
barplot = sns.barplot(data=df_plot, x='years', y=x, color='gray') #給定x軸,y軸和顏色繪製長條圖
...
# 設定標題和標籤
plt.title(x, fontsize=14)    #設定圖表標題
plt.xlabel("years", fontsize=12) #設定x軸標籤    
